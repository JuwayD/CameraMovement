...
namespace CameraMovement{
    public class Control_C_CinemachineStoryboard_Field :ICameraMovementControlField<Cinemachine.CinemachineStoryboard>
    {
       ...
       [UnityEngine.TooltipAttribute("How far away from the camera is the Canvas generated.")]
        ...
        public float m_PlaneDistanceDiff;
        ...
        public void ControlCinemachine(ref Cinemachine.CinemachineStoryboard target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            {
                var targetValue = (m_Alpha.IsExpression ? m_Alpha.Value : m_Alpha.PrimitiveValue);
                target.m_Alpha = Mathf.Approximately(0, templateDict[m_Alpha.Id].Config.duration) ? targetValue : m_AlphaAlertInit + templateDict[m_Alpha.Id].Config.alertCurve.Evaluate(templateDict[m_Alpha.Id].CostTime / templateDict[m_Alpha.Id].Config.duration) * m_AlphaDiff;
            }
            ...
            {
                var targetValue = (m_SplitView.IsExpression ? m_SplitView.Value : m_SplitView.PrimitiveValue);
                target.m_SplitView = Mathf.Approximately(0, templateDict[m_SplitView.Id].Config.duration) ? targetValue : m_SplitViewAlertInit + templateDict[m_SplitView.Id].Config.alertCurve.Evaluate(templateDict[m_SplitView.Id].CostTime / templateDict[m_SplitView.Id].Config.duration) * m_SplitViewDiff;
            }
            ...
            {
                var targetValue = (m_PlaneDistance.IsExpression ? m_PlaneDistance.Value : m_PlaneDistance.PrimitiveValue);
                target.m_PlaneDistance = Mathf.Approximately(0, templateDict[m_PlaneDistance.Id].Config.duration) ? targetValue : m_PlaneDistanceAlertInit + templateDict[m_PlaneDistance.Id].Config.alertCurve.Evaluate(templateDict[m_PlaneDistance.Id].CostTime / templateDict[m_PlaneDistance.Id].Config.duration) * m_PlaneDistanceDiff;
            }
        }
    }
}
