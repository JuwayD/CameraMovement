...
namespace CameraMovement{
    public class Control_CinemachineRecomposer_Field :ICameraMovementControlField<CinemachineRecomposer>
    {
       ...
       [UnityEngine.TooltipAttribute("Lowering this value relaxes the camera's attention to the Follow target (normal = 1)")]
        ...
        public float m_FollowAttachmentDiff;
       [UnityEngine.TooltipAttribute("Lowering this value relaxes the camera's attention to the LookAt target (normal = 1)")]
        ...
        public float m_LookAtAttachmentDiff;
        ...
        public void ControlCinemachine(ref CinemachineRecomposer target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            {
                var targetValue = (m_Tilt.IsExpression ? m_Tilt.Value : m_Tilt.PrimitiveValue);
                target.m_Tilt = Mathf.Approximately(0, templateDict[m_Tilt.Id].Config.duration) ? targetValue : m_TiltAlertInit + templateDict[m_Tilt.Id].Config.alertCurve.Evaluate(templateDict[m_Tilt.Id].CostTime / templateDict[m_Tilt.Id].Config.duration) * m_TiltDiff;
            }
            if (m_Pan.IsUse && templateDict.ContainsKey(m_Pan.Id))
            {
                var targetValue = (m_Pan.IsExpression ? m_Pan.Value : m_Pan.PrimitiveValue);
                target.m_Pan = Mathf.Approximately(0, templateDict[m_Pan.Id].Config.duration) ? targetValue : m_PanAlertInit + templateDict[m_Pan.Id].Config.alertCurve.Evaluate(templateDict[m_Pan.Id].CostTime / templateDict[m_Pan.Id].Config.duration) * m_PanDiff;
            }
            if (m_Dutch.IsUse && templateDict.ContainsKey(m_Dutch.Id))
            {
                var targetValue = (m_Dutch.IsExpression ? m_Dutch.Value : m_Dutch.PrimitiveValue);
                target.m_Dutch = Mathf.Approximately(0, templateDict[m_Dutch.Id].Config.duration) ? targetValue : m_DutchAlertInit + templateDict[m_Dutch.Id].Config.alertCurve.Evaluate(templateDict[m_Dutch.Id].CostTime / templateDict[m_Dutch.Id].Config.duration) * m_DutchDiff;
            }
            if (m_ZoomScale.IsUse && templateDict.ContainsKey(m_ZoomScale.Id))
            {
                var targetValue = (m_ZoomScale.IsExpression ? m_ZoomScale.Value : m_ZoomScale.PrimitiveValue);
                target.m_ZoomScale = Mathf.Approximately(0, templateDict[m_ZoomScale.Id].Config.duration) ? targetValue : m_ZoomScaleAlertInit + templateDict[m_ZoomScale.Id].Config.alertCurve.Evaluate(templateDict[m_ZoomScale.Id].CostTime / templateDict[m_ZoomScale.Id].Config.duration) * m_ZoomScaleDiff;
            }
            if (m_FollowAttachment.IsUse && templateDict.ContainsKey(m_FollowAttachment.Id))
            {
                var targetValue = (m_FollowAttachment.IsExpression ? m_FollowAttachment.Value : m_FollowAttachment.PrimitiveValue);
                target.m_FollowAttachment = Mathf.Approximately(0, templateDict[m_FollowAttachment.Id].Config.duration) ? targetValue : m_FollowAttachmentAlertInit + templateDict[m_FollowAttachment.Id].Config.alertCurve.Evaluate(templateDict[m_FollowAttachment.Id].CostTime / templateDict[m_FollowAttachment.Id].Config.duration) * m_FollowAttachmentDiff;
            }
            if (m_LookAtAttachment.IsUse && templateDict.ContainsKey(m_LookAtAttachment.Id))
            {
                var targetValue = (m_LookAtAttachment.IsExpression ? m_LookAtAttachment.Value : m_LookAtAttachment.PrimitiveValue);
                target.m_LookAtAttachment = Mathf.Approximately(0, templateDict[m_LookAtAttachment.Id].Config.duration) ? targetValue : m_LookAtAttachmentAlertInit + templateDict[m_LookAtAttachment.Id].Config.alertCurve.Evaluate(templateDict[m_LookAtAttachment.Id].CostTime / templateDict[m_LookAtAttachment.Id].Config.duration) * m_LookAtAttachmentDiff;
            }
        }
    }
}
