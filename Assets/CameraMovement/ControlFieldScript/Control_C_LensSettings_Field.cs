    ...
    public class Control_C_LensSettings_Field :ICameraMovementControlField<Cinemachine.LensSettings>
    {
       ...
       [UnityEngine.TooltipAttribute("Allows you to select a different camera mode to apply to the Camera component when Cinemachine activates this Virtual Camera.  The changes applied to the Camera component through this setting will remain after the Virtual Camera deactivation.")]
        ...
        public void AddByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority, ref Cinemachine.LensSettings target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            CameraMovement.Control_C_LensSettings_Config source = (CameraMovement.Control_C_LensSettings_Config)sourceConfig;
                if(source.FieldOfView.IsUse)
                {
                   ...
                   FieldOfViewAlertInit = target.FieldOfView - templateDict[FieldOfView.Id].Config.alertCurve.Evaluate(templateDict[FieldOfView.Id].CostTime / templateDict[FieldOfView.Id].Config.duration) * (targetValue - FieldOfViewAlertInit);
                }
                if(source.OrthographicSize.IsUse)
                {
                   ...
                   OrthographicSizeAlertInit = target.OrthographicSize - templateDict[OrthographicSize.Id].Config.alertCurve.Evaluate(templateDict[OrthographicSize.Id].CostTime / templateDict[OrthographicSize.Id].Config.duration) * (targetValue - OrthographicSizeAlertInit);
                }
                if(source.NearClipPlane.IsUse)
                {
                   ...
                   NearClipPlaneAlertInit = target.NearClipPlane - templateDict[NearClipPlane.Id].Config.alertCurve.Evaluate(templateDict[NearClipPlane.Id].CostTime / templateDict[NearClipPlane.Id].Config.duration) * (targetValue - NearClipPlaneAlertInit);
                }
                if(source.FarClipPlane.IsUse)
                {
                   ...
                   FarClipPlaneAlertInit = target.FarClipPlane - templateDict[FarClipPlane.Id].Config.alertCurve.Evaluate(templateDict[FarClipPlane.Id].CostTime / templateDict[FarClipPlane.Id].Config.duration) * (targetValue - FarClipPlaneAlertInit);
                }
                if(source.Dutch.IsUse)
                {
                   ...
                   DutchAlertInit = target.Dutch - templateDict[Dutch.Id].Config.alertCurve.Evaluate(templateDict[Dutch.Id].CostTime / templateDict[Dutch.Id].Config.duration) * (targetValue - DutchAlertInit);
                }
                ...
                if(source.FocusDistance.IsUse)
                {
                   ...
                   FocusDistanceAlertInit = target.FocusDistance - templateDict[FocusDistance.Id].Config.alertCurve.Evaluate(templateDict[FocusDistance.Id].CostTime / templateDict[FocusDistance.Id].Config.duration) * (targetValue - FocusDistanceAlertInit);
                }
        }
        ...
        public void ControlCinemachine(ref Cinemachine.LensSettings target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            {
                var targetValue = (FieldOfView.IsExpression ? FieldOfView.Value : FieldOfView.PrimitiveValue);
                target.FieldOfView = Mathf.Approximately(0, templateDict[FieldOfView.Id].Config.duration) ? targetValue : FieldOfViewAlertInit + templateDict[FieldOfView.Id].Config.alertCurve.Evaluate(templateDict[FieldOfView.Id].CostTime / templateDict[FieldOfView.Id].Config.duration) * (targetValue - FieldOfViewAlertInit);
            }
            if (OrthographicSize.IsUse && templateDict.ContainsKey(OrthographicSize.Id))
            {
                var targetValue = (OrthographicSize.IsExpression ? OrthographicSize.Value : OrthographicSize.PrimitiveValue);
                target.OrthographicSize = Mathf.Approximately(0, templateDict[OrthographicSize.Id].Config.duration) ? targetValue : OrthographicSizeAlertInit + templateDict[OrthographicSize.Id].Config.alertCurve.Evaluate(templateDict[OrthographicSize.Id].CostTime / templateDict[OrthographicSize.Id].Config.duration) * (targetValue - OrthographicSizeAlertInit);
            }
            if (NearClipPlane.IsUse && templateDict.ContainsKey(NearClipPlane.Id))
            {
                var targetValue = (NearClipPlane.IsExpression ? NearClipPlane.Value : NearClipPlane.PrimitiveValue);
                target.NearClipPlane = Mathf.Approximately(0, templateDict[NearClipPlane.Id].Config.duration) ? targetValue : NearClipPlaneAlertInit + templateDict[NearClipPlane.Id].Config.alertCurve.Evaluate(templateDict[NearClipPlane.Id].CostTime / templateDict[NearClipPlane.Id].Config.duration) * (targetValue - NearClipPlaneAlertInit);
            }
            if (FarClipPlane.IsUse && templateDict.ContainsKey(FarClipPlane.Id))
            {
                var targetValue = (FarClipPlane.IsExpression ? FarClipPlane.Value : FarClipPlane.PrimitiveValue);
                target.FarClipPlane = Mathf.Approximately(0, templateDict[FarClipPlane.Id].Config.duration) ? targetValue : FarClipPlaneAlertInit + templateDict[FarClipPlane.Id].Config.alertCurve.Evaluate(templateDict[FarClipPlane.Id].CostTime / templateDict[FarClipPlane.Id].Config.duration) * (targetValue - FarClipPlaneAlertInit);
            }
            if (Dutch.IsUse && templateDict.ContainsKey(Dutch.Id))
            {
                var targetValue = (Dutch.IsExpression ? Dutch.Value : Dutch.PrimitiveValue);
                target.Dutch = Mathf.Approximately(0, templateDict[Dutch.Id].Config.duration) ? targetValue : DutchAlertInit + templateDict[Dutch.Id].Config.alertCurve.Evaluate(templateDict[Dutch.Id].CostTime / templateDict[Dutch.Id].Config.duration) * (targetValue - DutchAlertInit);
            }
            ...
            {
                var targetValue = (FocusDistance.IsExpression ? FocusDistance.Value : FocusDistance.PrimitiveValue);
                target.FocusDistance = Mathf.Approximately(0, templateDict[FocusDistance.Id].Config.duration) ? targetValue : FocusDistanceAlertInit + templateDict[FocusDistance.Id].Config.alertCurve.Evaluate(templateDict[FocusDistance.Id].CostTime / templateDict[FocusDistance.Id].Config.duration) * (targetValue - FocusDistanceAlertInit);
            }
        }
    }
}
