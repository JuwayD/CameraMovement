        ...
        public class Control_C_CIL_ImpulseReaction_Field :ICameraMovementControlField<Cinemachine.CinemachineImpulseListener.ImpulseReaction>
    {
       ...
       [UnityEngine.TooltipAttribute("How long the secondary reaction lasts.")]
        ...
        public void AddByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority)
        {
            ...
            if(source.m_Duration.IsUse) m_Duration.Add(new MixItem<System.Single>(id, priority, source.m_Duration.CalculatorExpression, source.m_Duration.Value, source.m_Duration.IsUse));
        }
        public void RemoveByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority)
        {
            ...
            if(source.m_Duration.IsUse) m_Duration.Remove(new MixItem<System.Single>(id, priority, source.m_Duration.CalculatorExpression, source.m_Duration.Value, source.m_Duration.IsUse));
        }
        ...
        public void ControlCinemachine(ref Cinemachine.CinemachineImpulseListener.ImpulseReaction target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            if (m_Duration.IsUse && templateDict.ContainsKey(m_Duration.Id))
                target.m_Duration = Mathf.Approximately(0, templateDict[m_Duration.Id].Config.duration) ? (m_Duration.IsExpression ? m_Duration.Value : m_Duration.PrimitiveValue) : templateDict[m_Duration.Id].Config.alertCurve.Evaluate(templateDict[m_Duration.Id].CostTime / templateDict[m_Duration.Id].Config.duration) * (m_Duration.IsExpression ? m_Duration.Value : m_Duration.PrimitiveValue);
        }
    }
}
