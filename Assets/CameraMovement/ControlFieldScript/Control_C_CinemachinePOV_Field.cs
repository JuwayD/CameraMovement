        ...
        public class Control_C_CinemachinePOV_Field :ICameraMovementControlField
    {
       ...
       [UnityEngine.HideInInspector]
        ...
        public void AddByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority)
        {
            ...
            if(source.m_RecenterTarget.IsUse) m_RecenterTarget.Add(new MixItem<Cinemachine.CinemachinePOV.RecenterTargetMode>(id, priority, source.m_RecenterTarget.Value));
            ...
            if(source.m_ApplyBeforeBody.IsUse) m_ApplyBeforeBody.Add(new MixItem<System.Boolean>(id, priority, source.m_ApplyBeforeBody.Value));
        }
        public void RemoveByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority)
        {
            if(sourceConfig.GetType() != AttachControlField) return;
            CameraMovement.Control_C_CinemachinePOV_Config source = (CameraMovement.Control_C_CinemachinePOV_Config)sourceConfig;
            if(source.m_RecenterTarget.IsUse) m_RecenterTarget.Remove(new MixItem<Cinemachine.CinemachinePOV.RecenterTargetMode>(id, priority, source.m_RecenterTarget.Value));
            ...
            if(source.m_ApplyBeforeBody.IsUse) m_ApplyBeforeBody.Remove(new MixItem<System.Boolean>(id, priority, source.m_ApplyBeforeBody.Value));
        }
        public void ControlCinemachine(object targetObj, Dictionary<int, RuntimeTemplate> templateDict)
        {
            var target = (Cinemachine.CinemachinePOV)targetObj;
            target.m_RecenterTarget = m_RecenterTarget.Value;
            // 处理字段 m_VerticalAxis
            // 生成递归代码
            ...
            target.m_ApplyBeforeBody = m_ApplyBeforeBody.Value;
        }
    }
}
