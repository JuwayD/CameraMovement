       ...
       public  Type AttachControlField => typeof(Cinemachine.CinemachineFreeLook.Orbit);
        ...
        public void AddByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority, ref Cinemachine.CinemachineFreeLook.Orbit target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            if(source.m_Height.IsUse)
            {
               ...
               m_HeightAlertInit = target.m_Height - templateDict[m_Height.Id].Config.alertCurve.Evaluate(templateDict[m_Height.Id].CostTime / templateDict[m_Height.Id].Config.duration) * (m_HeightDiff);
            }
            if(source.m_Radius.IsUse)
            {
               ...
               m_RadiusAlertInit = target.m_Radius - templateDict[m_Radius.Id].Config.alertCurve.Evaluate(templateDict[m_Radius.Id].CostTime / templateDict[m_Radius.Id].Config.duration) * (m_RadiusDiff);
            }
        }
