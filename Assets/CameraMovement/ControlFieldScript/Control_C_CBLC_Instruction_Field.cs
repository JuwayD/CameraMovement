        ...
        public class Control_C_CBLC_Instruction_Field :ICameraMovementControlField
    {
       ...
       [UnityEngine.TooltipAttribute("How to blend to the next virtual camera in the list (if any)")]
        ...
        public void AddByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority)
        {
            ...
            if(source.m_Hold.IsUse) m_Hold.Add(new MixItem<System.Single>(id, priority, source.m_Hold.CalculatorExpression, source.m_Hold.Value));
        ...
        public void RemoveByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority)
        {
            if(sourceConfig.GetType() != AttachControlField) return;
            CameraMovement.Control_C_CBLC_Instruction_Config source = (CameraMovement.Control_C_CBLC_Instruction_Config)sourceConfig;
            if(source.m_Hold.IsUse) m_Hold.Remove(new MixItem<System.Single>(id, priority, source.m_Hold.CalculatorExpression, source.m_Hold.Value));
        ...
        public void ControlCinemachine(object targetObj, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            if (templateDict.ContainsKey(m_Hold.Id))
                target.m_Hold = templateDict[m_Hold.Id].Config.alertCurve.Evaluate(templateDict[m_Hold.Id].CostTime / templateDict[m_Hold.Id].Config.duration) * m_Hold.Value;
            target.m_Hold = (System.Single)m_Hold.Value;
            // 处理字段 m_Blend
            // 生成递归代码
