...
namespace CameraMovement{
    public class Control_C_AxisState_Field :ICameraMovementControlField<Cinemachine.AxisState>
    {
       ...
       [UnityEngine.TooltipAttribute("The value of the input axis.  A value of 0 means no input.  You can drive this directly from a custom input system, or you can set the Axis Name and have the value driven by the internal Input Manager")]
        ...
        public float m_InputAxisValueDiff;
       ...
       [UnityEngine.TooltipAttribute("Automatic recentering to at-rest position")]
        ...
        public void AddByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority, ref Cinemachine.AxisState target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            m_Recentering?.AddByConfig(source.m_Recentering, id, priority, ref target.m_Recentering, templateDict);
        }
        public void RemoveByConfig(CameraMovementControlConfigBase sourceConfig,int id,int priority, ref Cinemachine.AxisState target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            if(sourceConfig == null) return;
            if(sourceConfig.AttachControlField != AttachControlField) return;
            CameraMovement.Control_C_AxisState_Config source = (CameraMovement.Control_C_AxisState_Config)sourceConfig;
            ...
            m_Recentering?.RemoveByConfig(source.m_Recentering, id, priority, ref target.m_Recentering, templateDict);
        }
        ...
        public void ControlCinemachine(ref Cinemachine.AxisState target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            {
                var targetValue = (Value.IsExpression ? Value.Value : Value.PrimitiveValue);
                target.Value = Mathf.Approximately(0, templateDict[Value.Id].Config.duration) ? targetValue : ValueAlertInit + templateDict[Value.Id].Config.alertCurve.Evaluate(templateDict[Value.Id].CostTime / templateDict[Value.Id].Config.duration) * ValueDiff;
            }
            ...
            {
                var targetValue = (m_MaxSpeed.IsExpression ? m_MaxSpeed.Value : m_MaxSpeed.PrimitiveValue);
                target.m_MaxSpeed = Mathf.Approximately(0, templateDict[m_MaxSpeed.Id].Config.duration) ? targetValue : m_MaxSpeedAlertInit + templateDict[m_MaxSpeed.Id].Config.alertCurve.Evaluate(templateDict[m_MaxSpeed.Id].CostTime / templateDict[m_MaxSpeed.Id].Config.duration) * m_MaxSpeedDiff;
            }
            if (m_AccelTime.IsUse && templateDict.ContainsKey(m_AccelTime.Id))
            {
                var targetValue = (m_AccelTime.IsExpression ? m_AccelTime.Value : m_AccelTime.PrimitiveValue);
                target.m_AccelTime = Mathf.Approximately(0, templateDict[m_AccelTime.Id].Config.duration) ? targetValue : m_AccelTimeAlertInit + templateDict[m_AccelTime.Id].Config.alertCurve.Evaluate(templateDict[m_AccelTime.Id].CostTime / templateDict[m_AccelTime.Id].Config.duration) * m_AccelTimeDiff;
            }
            if (m_DecelTime.IsUse && templateDict.ContainsKey(m_DecelTime.Id))
            {
                var targetValue = (m_DecelTime.IsExpression ? m_DecelTime.Value : m_DecelTime.PrimitiveValue);
                target.m_DecelTime = Mathf.Approximately(0, templateDict[m_DecelTime.Id].Config.duration) ? targetValue : m_DecelTimeAlertInit + templateDict[m_DecelTime.Id].Config.alertCurve.Evaluate(templateDict[m_DecelTime.Id].CostTime / templateDict[m_DecelTime.Id].Config.duration) * m_DecelTimeDiff;
            }
            ...
            {
                var targetValue = (m_InputAxisValue.IsExpression ? m_InputAxisValue.Value : m_InputAxisValue.PrimitiveValue);
                target.m_InputAxisValue = Mathf.Approximately(0, templateDict[m_InputAxisValue.Id].Config.duration) ? targetValue : m_InputAxisValueAlertInit + templateDict[m_InputAxisValue.Id].Config.alertCurve.Evaluate(templateDict[m_InputAxisValue.Id].CostTime / templateDict[m_InputAxisValue.Id].Config.duration) * m_InputAxisValueDiff;
            }
            ...
            {
                var targetValue = (m_MinValue.IsExpression ? m_MinValue.Value : m_MinValue.PrimitiveValue);
                target.m_MinValue = Mathf.Approximately(0, templateDict[m_MinValue.Id].Config.duration) ? targetValue : m_MinValueAlertInit + templateDict[m_MinValue.Id].Config.alertCurve.Evaluate(templateDict[m_MinValue.Id].CostTime / templateDict[m_MinValue.Id].Config.duration) * m_MinValueDiff;
            }
            if (m_MaxValue.IsUse && templateDict.ContainsKey(m_MaxValue.Id))
            {
                var targetValue = (m_MaxValue.IsExpression ? m_MaxValue.Value : m_MaxValue.PrimitiveValue);
                target.m_MaxValue = Mathf.Approximately(0, templateDict[m_MaxValue.Id].Config.duration) ? targetValue : m_MaxValueAlertInit + templateDict[m_MaxValue.Id].Config.alertCurve.Evaluate(templateDict[m_MaxValue.Id].CostTime / templateDict[m_MaxValue.Id].Config.duration) * m_MaxValueDiff;
            }
