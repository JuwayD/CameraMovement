    ...
    public class Control_C_CinemachineCollider_Field :ICameraMovementControlField<Cinemachine.CinemachineCollider>
    {
       ...
       [UnityEngine.HeaderAttribute("Shot Evaluation")]
        ...
        public void ControlCinemachine(ref Cinemachine.CinemachineCollider target, Dictionary<int, RuntimeTemplate> templateDict)
        {
            ...
            {
                var targetValue = (m_MinimumDistanceFromTarget.IsExpression ? m_MinimumDistanceFromTarget.Value : m_MinimumDistanceFromTarget.PrimitiveValue);
                target.m_MinimumDistanceFromTarget = Mathf.Approximately(0, templateDict[m_MinimumDistanceFromTarget.Id].Config.duration) ? targetValue : m_MinimumDistanceFromTargetAlertInit + templateDict[m_MinimumDistanceFromTarget.Id].Config.alertCurve.Evaluate(templateDict[m_MinimumDistanceFromTarget.Id].CostTime / templateDict[m_MinimumDistanceFromTarget.Id].Config.duration) * (targetValue - m_MinimumDistanceFromTargetAlertInit);
            }
            ...
            {
                var targetValue = (m_DistanceLimit.IsExpression ? m_DistanceLimit.Value : m_DistanceLimit.PrimitiveValue);
                target.m_DistanceLimit = Mathf.Approximately(0, templateDict[m_DistanceLimit.Id].Config.duration) ? targetValue : m_DistanceLimitAlertInit + templateDict[m_DistanceLimit.Id].Config.alertCurve.Evaluate(templateDict[m_DistanceLimit.Id].CostTime / templateDict[m_DistanceLimit.Id].Config.duration) * (targetValue - m_DistanceLimitAlertInit);
            }
            if (m_MinimumOcclusionTime.IsUse && templateDict.ContainsKey(m_MinimumOcclusionTime.Id))
            {
                var targetValue = (m_MinimumOcclusionTime.IsExpression ? m_MinimumOcclusionTime.Value : m_MinimumOcclusionTime.PrimitiveValue);
                target.m_MinimumOcclusionTime = Mathf.Approximately(0, templateDict[m_MinimumOcclusionTime.Id].Config.duration) ? targetValue : m_MinimumOcclusionTimeAlertInit + templateDict[m_MinimumOcclusionTime.Id].Config.alertCurve.Evaluate(templateDict[m_MinimumOcclusionTime.Id].CostTime / templateDict[m_MinimumOcclusionTime.Id].Config.duration) * (targetValue - m_MinimumOcclusionTimeAlertInit);
            }
            if (m_CameraRadius.IsUse && templateDict.ContainsKey(m_CameraRadius.Id))
            {
                var targetValue = (m_CameraRadius.IsExpression ? m_CameraRadius.Value : m_CameraRadius.PrimitiveValue);
                target.m_CameraRadius = Mathf.Approximately(0, templateDict[m_CameraRadius.Id].Config.duration) ? targetValue : m_CameraRadiusAlertInit + templateDict[m_CameraRadius.Id].Config.alertCurve.Evaluate(templateDict[m_CameraRadius.Id].CostTime / templateDict[m_CameraRadius.Id].Config.duration) * (targetValue - m_CameraRadiusAlertInit);
            }
            ...
            {
                var targetValue = (m_SmoothingTime.IsExpression ? m_SmoothingTime.Value : m_SmoothingTime.PrimitiveValue);
                target.m_SmoothingTime = Mathf.Approximately(0, templateDict[m_SmoothingTime.Id].Config.duration) ? targetValue : m_SmoothingTimeAlertInit + templateDict[m_SmoothingTime.Id].Config.alertCurve.Evaluate(templateDict[m_SmoothingTime.Id].CostTime / templateDict[m_SmoothingTime.Id].Config.duration) * (targetValue - m_SmoothingTimeAlertInit);
            }
            ...
            {
                var targetValue = (m_DampingWhenOccluded.IsExpression ? m_DampingWhenOccluded.Value : m_DampingWhenOccluded.PrimitiveValue);
                target.m_DampingWhenOccluded = Mathf.Approximately(0, templateDict[m_DampingWhenOccluded.Id].Config.duration) ? targetValue : m_DampingWhenOccludedAlertInit + templateDict[m_DampingWhenOccluded.Id].Config.alertCurve.Evaluate(templateDict[m_DampingWhenOccluded.Id].CostTime / templateDict[m_DampingWhenOccluded.Id].Config.duration) * (targetValue - m_DampingWhenOccludedAlertInit);
            }
            if (m_OptimalTargetDistance.IsUse && templateDict.ContainsKey(m_OptimalTargetDistance.Id))
            {
                var targetValue = (m_OptimalTargetDistance.IsExpression ? m_OptimalTargetDistance.Value : m_OptimalTargetDistance.PrimitiveValue);
                target.m_OptimalTargetDistance = Mathf.Approximately(0, templateDict[m_OptimalTargetDistance.Id].Config.duration) ? targetValue : m_OptimalTargetDistanceAlertInit + templateDict[m_OptimalTargetDistance.Id].Config.alertCurve.Evaluate(templateDict[m_OptimalTargetDistance.Id].CostTime / templateDict[m_OptimalTargetDistance.Id].Config.duration) * (targetValue - m_OptimalTargetDistanceAlertInit);
            }
        }
    }
}
